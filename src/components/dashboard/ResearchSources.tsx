...
export const ResearchSources = () => {
  ...
  useEffect(() => {
    const fetchSources = async () => {
      ...
      const {
        data
      } = await supabase.from('research_reports').select('sources, report_date').gte('report_date', dateFilter).order('report_date', {
        ascending: false
      });
    ...
    fetchSources();
    const channel = supabase.channel('research-sources-changes').on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'research_reports'
    }, () => fetchSources()).subscribe();
    return () => {
  ...
  const isPortugueseDomain = (url: string) => {
    try {
      ...
      return domain.endsWith('.pt') || domain.includes('observador') || domain.includes('publico') || domain.includes('expresso') || domain.includes('visao') || domain.includes('jornaldenegocios');
    } catch {
      return false;
    }
  };
  ...
  const isFighterRelated = (url: string) => {
    const fighterKeywords = ['gripen', 'f-35', 'f35', 'fighter', 'aircraft', 'aviao', 'aviação', 'defesa', 'defense', 'militar', 'military', 'forca aerea', 'air force', 'procurement', 'aquisição', 'saab', 'lockheed', 'nato', 'otan'];
