...
export const SentimentDashboard = ({
  ...
  return <div className="space-y-6">
      ...
      {/* Sentiment Breakdown */}
      {mediaSentiment && <Card className="p-6">
          <h3 className="text-lg font-semibold mb-4">Sentiment Breakdown</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {Object.entries(mediaSentiment).map(([fighter, data]) => <div key={fighter} className="p-4 border rounded-lg space-y-2">
      ...
      {socialPosts && socialPosts.length > 0 && <Card className="p-6">
          <h3 className="text-lg font-semibold mb-4">Latest Social Media Posts</h3>
          <div className="space-y-3">
            {socialPosts.slice(0, 10).map((post, idx) => <a key={idx} href={post.post_url} target="_blank" rel="noopener noreferrer" className="flex items-start gap-3 p-3 border rounded-lg hover:bg-accent transition-colors">
      ...
      <Card className="p-6">
        ...
        {articles.length > 0 ? <div className="space-y-3">
            {articles.map((article, idx) => <a key={idx} href={article.url} target="_blank" rel="noopener noreferrer" className="flex items-start gap-3 p-3 border rounded-lg hover:bg-accent transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex gap-2 flex-wrap items-center">
                    {article.source_country && <Badge variant={article.source_country === activeCountry ? "default" : "outline"} className="text-xs">
                        {article.source_country === activeCountry ? "Local" : article.source_country}
                      </Badge>}
                    {article.fighter_tags?.map((tag: string) => <Badge key={tag} variant="secondary" className="text-xs">
                        {tag}
                      </Badge>)}
                    {article.sentiment !== null && <div className="flex items-center gap-1">
